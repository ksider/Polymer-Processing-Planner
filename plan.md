...
## 12) План развития Messenger

Цель: довести текущую систему сообщений до состояния рабочего внутреннего мессенджера, не ломая уже собранную модель `direct/group/system`, а развивая ее поэтапно. Приоритет: сначала улучшить UX и читаемость интерфейса, затем наращивать функциональность.

### 12.0 Текущее состояние на сейчас
1. Уже реализовано:
1. отдельная страница messenger с фиксированными зонами `chat list / thread / composer`;
2. правая колонка `Notifications` только для системных сообщений;
3. direct/group/system модель комнат;
4. drafts на комнату;
5. reply-to с переходом к исходному сообщению;
6. pinned messages;
7. edit своих сообщений + history;
8. unread separator `New messages`;
9. поиск по чатам и по сообщениям;
10. attachment picker по сущностям (`experiment / qualification_step / doe / report`) и attachment-card в сообщении;
11. групповые чаты с управлением участниками;
12. generated avatars + настройки аватара в профиле;
13. реакции с ограничением `1 user -> 1 reaction per message`;
14. tooltip со списком пользователей для summary реакций.
2. Уже убрано/зафиксировано:
1. системные нотификации не смешиваются с обычными чатами;
2. фильтр `system` в header обычного чата скрыт как лишний UI;
3. системные сообщения живут в правой rail-колонке, а не в основном chat-list.
3. Что еще нестабильно и требует доводки:
1. финальная геометрия/анимация reactions rail;
2. доведение attachment UX;
3. действия `message -> task/calendar`.

### 12.1 Этап 1 — UI/UX сначала
1. Поиск по чатам и сообщениям:
1. быстрый фильтр списка чатов;
2. поиск внутри текущего чата;
3. режимы `all / mentions / attachments` в header чата.
2. Разделители и навигация по непрочитанному:
1. линия `New messages` в треде;
2. быстрый переход к первому непрочитанному;
3. нормальная индикация позиции в длинной переписке.
3. Drafts на чат:
1. сохранять незавершенный текст отдельно для каждой комнаты;
2. восстанавливать draft при возврате в чат;
3. показывать индикацию `Draft` в списке чатов.
4. Reply-to и цитирование:
1. ответ на конкретное сообщение;
2. компактный preview цитаты над composer;
3. переход к исходному сообщению по клику на reply-block.
5. Pinned messages:
1. закрепленные сообщения в чате;
2. компактный rail/header-block для pinned;
3. быстрый переход к закрепленному сообщению.
6. Редактирование своих сообщений:
1. edit для собственных сообщений;
2. mark `edited`;
3. история правок для прозрачности.
7. Доводка уже реализованного UI:
1. упростить и стабилизировать reactions rail в свободной зоне напротив аватара;
2. сделать tooltip/hover-поведение реакций полностью предсказуемым;
3. довести плотность и адаптивность thread header/composer/sidebar.

### 12.2 Этап 2 — структурированные вложения и ссылки
1. Уже есть MVP attachment-card и attach picker по сущностям.
2. Следующий шаг: заменить одиночный `payload/path` на нормальную модель вложений.
3. Добавить `message_attachments`:
1. `message_id`,
2. `attachment_type`,
3. `entity_type`,
4. `entity_id`,
...
1. `task`,
2. `experiment`,
3. `qualification_step`,
4. `doe`,
5. `report`,
6. `calendar_event`,
7. `link`.
5. В UI:
1. attachment-card под сообщением;
2. единый внешний вид для вложений и системных ссылок;
3. кнопка `Attach` в composer с picker по сущностям.
4. Дерево сущностей по умолчанию свернуто; дальше нужна только UX-полировка, не смена паттерна.

### 12.3 Этап 3 — легкие социальные механики
1. Реакции уже реализованы как отдельная боковая зона сообщения.
2. Таблица `message_reactions`:
1. `message_id`,
2. `user_id`,
3. `emoji`,
4. `created_at`,
5. unique `(message_id, user_id)` с переключением типа реакции.
3. Текущий фиксированный набор:
1. `👍`,
2. `✅`,
3. `👀`,
4. `⚠️`,
...
6. `⛔`.
4. Уже есть в UI:
1. quick reaction around central trigger;
2. отображение нескольких разных реакций со своими счетчиками;
3. tooltip со списком пользователей.
5. Что осталось по реакциям:
1. довести финальную геометрию ring-menu;
2. при необходимости добавить soft animation и упрощенный mobile fallback.

### 12.4 Этап 4 — действия над сообщениями
1. Создание задачи из сообщения (`Create task from message`):
1. не переносить сообщение, а создавать связанную задачу;
2. хранить ссылку на исходное сообщение;
3. в модалке давать выбор сущности-контейнера, assignee, deadline.
2. Правило выбора “нужного таск-менеджера”:
1. если у сообщения уже есть attachment-сущность, предлагать ее первой;
2. если пользователь открыл мессенджер из контекста сущности в будущем, подставлять ее автоматически;
3. если контекста нет, открывать entity picker (`experiment / qualification step / doe / report`).
3. Создание календарного события из сообщения (`Add to calendar`):
1. title, date/time, duration, participants, related entity;
2. хранить ссылку на исходное сообщение;
3. открывать запись календаря по клику обратно в чат.

### 12.5 Рекомендуемый порядок реализации
1. Дожать текущий UI/UX messenger:
1. финализировать reactions rail;
2. довести attachment UX;
3. вычистить адаптивность и плотность.
2. Структурированные вложения:
1. новая модель `message_attachments`,
2. support нескольких attachments,
3. draft persistence для attachments.
3. `Create task from message`.
4. `Add to calendar from message`.
5. После этого уже мелкие улучшения и polish.

### 12.6 Критерий готовности следующей версии messenger
1. Длинные переписки читаются без потери контекста.
2. У сообщения есть не только текст, но и структурированные действия/связи.
3. Реакции не ломают layout, живут в отдельной свободной зоне и нормально работают с hover/tooltip.
4. Любое важное сообщение можно быстро:
1. закрепить,
2. процитировать,
3. превратить в задачу,
4. превратить в календарное событие.
5. System notifications и chat-механика остаются единым визуальным языком, но разными по назначению.
