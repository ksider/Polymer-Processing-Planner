...
3. режимы `all / mentions / attachments / system`.
...
1. Заменить одиночный `payload/path` на нормальную модель вложений.
2. Добавить `message_attachments`:
1. `message_id`,
2. `attachment_type`,
3. `entity_type`,
4. `entity_id`,
...
1. Реакции под сообщениями.
2. Таблица `message_reactions`:
1. `message_id`,
2. `user_id`,
3. `emoji`,
4. `created_at`,
5. unique `(message_id, user_id, emoji)`.
3. На первом этапе использовать фиксированный набор реакций:
1. `+1`,
2. `eyes`,
3. `check`,
4. `fire`,
5. `question`.
4. В UI:
1. quick reaction on hover;
2. группировка одинаковых реакций;
...
2. attachment-card,
3. entity picker в composer.
3. Реакции.
4. `Create task from message`.
5. `Add to calendar from message`.
...
3. Любое важное сообщение можно быстро:
1. закрепить,
2. процитировать,
3. превратить в задачу,
4. превратить в календарное событие.
4. System notifications и chat-механика остаются единым визуальным языком, но разными по назначению.
